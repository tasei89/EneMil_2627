...
def _list_teams_and_batters(target_year: str, _n: int,
...
@st.cache_data(show_spinner=False)
def _batter_date_range(_n: int, _df_all: pd.DataFrame, batter_col: str,
...
@st.cache_data(show_spinner=False)
def _build_batter_distributions(_n: int, _df_all: pd.DataFrame,
                                min_pa: int = 5) -> dict:
    """全打者の打者詳細指標・基本成績・初球・2ストライクの分布を作る。
    打者名 × 打者打席左右の組み合わせ単位で集計。
...
@st.cache_data(show_spinner=False)
def _build_batter_pitchtype_distributions(_n: int,
                                          _df_all: pd.DataFrame,
                                          min_pitches: int = 15) -> dict:
    """全打者の球種別セイバーから「球種×指標」分布を作る。"""
    df_all = _df_all
    batter_col = _pick_col(df_all, ["打者名", "BatterName", "打者"])
    if not batter_col:
        return {}
...
def render():
    ...
    with st.sidebar:
        ...
        if team_col:
            teams = tb["teams"]
            team = st.selectbox("① チーム選択", teams, key="bat_team")
        ...
        if not batters:
            ...
            return
        batter = st.selectbox("② 打者選択", batters, key="bat_player")
    ...
    dist = _build_batter_distributions(len(df_all), df_all)
    dist_pt = _build_batter_pitchtype_distributions(len(df_all), df_all)
