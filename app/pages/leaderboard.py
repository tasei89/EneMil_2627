...
def _build_pitcher_board_df(_n: int, _df: pd.DataFrame) -> pd.DataFrame:
    """投手・全球種モードの集計表（重い処理）をキャッシュ。
    _n: キャッシュキー用、_df: アンダースコア付きでハッシュをskip。"""
    df_all = _df
...
@st.cache_data(show_spinner=False)
def _build_pitchtype_board_df(_n: int, _df: pd.DataFrame,
...
@st.cache_data(show_spinner=False)
def _build_batter_board_df(_n: int, _df: pd.DataFrame) -> pd.DataFrame:
    """打者集計表（重い処理）をキャッシュ。"""
    df_all = _df
    ...
    with st.sidebar:
        ...
        if tab_type == "投手":
            ...
            min_pa = st.number_input("最低打席数", min_value=0,
                                     value=5, step=5, key="lb_min_b")
            target_pitch = "全球種"

    log_event("leaderboard_view", tab_type)

    if tab_type == "投手":
        _render_pitcher_board(df_year, int(min_pa), target_pitch)
    else:
        _render_batter_board(df_year, int(min_pa))
...
def _render_pitcher_board(df_year: pd.DataFrame, min_pa: int,
    ...
    if is_pitch_filtered:
        df_lb = _build_pitchtype_board_df(len(df_year), df_year, target_pitch)
    else:
        df_lb = _build_pitcher_board_df(len(df_year), df_year)
...
def _render_batter_board(df_year: pd.DataFrame, min_pa: int):
    df_lb = _build_batter_board_df(len(df_year), df_year)
