...
def _build_pitcher_board_df(target_year: str, _n: int,
                            _df: pd.DataFrame) -> pd.DataFrame:
    """投手・全球種モードの集計表（重い処理）をキャッシュ。
    target_year, _n をキャッシュキーに使う（_df はハッシュskip）。
    ★ target_year をキーに含めないと、行数が偶然一致する別の年の
       結果が誤って再利用されるバグになる。"""
    df_all = _df
...
@st.cache_data(show_spinner=False)
def _build_pitchtype_board_df(target_year: str, _n: int, _df: pd.DataFrame,
...
@st.cache_data(show_spinner=False)
def _build_batter_board_df(target_year: str, _n: int,
                           _df: pd.DataFrame) -> pd.DataFrame:
    """打者集計表（重い処理）をキャッシュ。
    ★ target_year をキーに含める（行数一致による誤キャッシュ防止）。"""
    df_all = _df
