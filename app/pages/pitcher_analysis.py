...
@st.cache_data(show_spinner=False)
def _build_pitcher_distributions(n: int, _df_all: pd.DataFrame,
                                 min_pitches: int = 20) -> dict:
    """全投手の基本成績・投手詳細指標から各指標の分布を作る。
    キャッシュキー: (n, min_pitches)。_df_all はハッシュ skip。
    ★ n を通常引数（非アンダースコア）にして、CSV更新時にキャッシュ自動 invalidate。"""
    df_all = _df_all
...
@st.cache_data(show_spinner=False)
def _build_pitchtype_distributions(n: int, _df_all: pd.DataFrame,
                                   min_pitches: int = 25) -> dict:
    """全投手の球種別の成績から、各「球種×指標」の分布を作る。
    キャッシュキー: (n, min_pitches)。"""
    df_all = _df_all
    pitcher_col = _pick_col(df_all, ["投手名"])
    if not pitcher_col:
        return {}
