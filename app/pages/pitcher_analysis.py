...
from ..stats.calculations import (
    ...
    _pick_col, _normalize_date_series, get_csv_fingerprint,
)
...
# ──────────────────────────────────────────
# 階層キャッシュ：
#   L1: モジュールレベル global  (_LEAGUE_CACHE)       … プロセス内 / 全ページ共通 / 数十μs
#   L2: ディスク (pickle)         data/.cache/*.pkl    … アプリ再起動後も復元 / 数百ms
#   L3: 計算 (純粋関数 _compute_*)                      … 数十秒〜数分
#
# L1 を使うことで Streamlit の @st.cache_data の内部判定オーバーヘッドを完全に排除。
# fingerprint をキーに使うので CSV 更新時に自動 invalidate される。

_LEAGUE_CACHE: dict = {}  # {(fingerprint, name): result}


def _get_or_build_dist(data_dir: str, name: str, builder,
                        df_all: pd.DataFrame, *args) -> dict:
    """L1(module) → L2(disk) → L3(compute) の順にフォールバックする。
    builder(df_all, *args) は dict を返す純粋関数。"""
    import pickle
...
def _list_teams_and_pitchers(target_year: str, n: int,
                             _df_year: pd.DataFrame,
                             team_col: str | None,
                             pitcher_col: str) -> dict:
    """対象年フィルタ済みデータから、チーム一覧と各チームの投手一覧を一度だけ抽出してキャッシュ。
    キャッシュキー: (target_year, n, team_col, pitcher_col)
    ★ n は必ずキャッシュキーに含める。アンダースコア接頭辞にすると Streamlit が
       キーから除外し、CSVが更新されても古い一覧を返し続けるバグになる。"""
...
@st.cache_data(show_spinner=False)
def _player_date_range(n: int, _df_all: pd.DataFrame, player_col: str,
                        player: str, target_year: str) -> tuple:
    """特定選手の日付範囲（start, end）をキャッシュ。
    キャッシュキー: (n, player_col, player, target_year)
    ★ n を通常引数（非アンダースコア）にして、CSV更新時に
       キャッシュが自動 invalidate されるようにする。"""
...
def _compute_pitcher_distributions(df_all: pd.DataFrame,
                                    min_pitches: int = 20) -> dict:
    """全投手の基本成績・投手詳細指標から各指標の分布を作る（純粋計算）。
    キャッシュは _disk_cached_dist 経由で行う。"""
...
def _compute_pitchtype_distributions(df_all: pd.DataFrame,
                                      min_pitches: int = 25) -> dict:
    """全投手の球種別の成績から、各「球種×指標」の分布を作る（純粋計算）。"""
    pitcher_col = _pick_col(df_all, ["投手名"])
    if not pitcher_col:
        return {}
    dist = {}
...
def render():
    ...
    if df.empty:
        ...
        #    日付フィルタを外して選手データだけで再試行
        df_no_date = filter_by_player_period(df_all, pitcher_col, pitcher,
                                              None, None)
        if df_no_date.empty:
            # 本当に選手データが存在しない
            st.warning(f"「{pitcher}」のデータが見つかりません。")
            return
        # 日付フィルタを外せばデータがある → 範囲外を選んでいる
        st.warning(
            f"指定された期間（{start_str} 〜 {end_str}）に「{pitcher}」のデータがありません。"
    ...
    # CSV 更新時は fingerprint が変わり、自動的に新規ファイル名で再計算される
    dist_pitcher = _get_or_build_dist(
        DATA_DIR, "dist_pitcher", _compute_pitcher_distributions, df_all)
    dist_pitchtype = _get_or_build_dist(
        DATA_DIR, "dist_pitchtype", _compute_pitchtype_distributions, df_all)
