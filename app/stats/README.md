...
def calculate_game_metrics(df: pd.DataFrame) -> dict:
    ...
    if "打席内球数" in df.columns:
        ...
        # 各打席の最終行の打席結果（last は NaN を無視するため fillna 後に取得）
        # ※ max(_pn) の行が打席結果を持つはずだが、念のため最終行を取る
        last_idx = grp.tail(1).set_index("_pa_id")
        pa_result_s = last_idx["_pa_res_str"]

        # 4球目（_pn==4）の S と、3球目（_pn==3）の post-S を pa_id ごとに引く
        row4 = d_tmp[d_tmp["_pn"] == 4].set_index("_pa_id")
        row3 = d_tmp[d_tmp["_pn"] == 3].set_index("_pa_id")
        s_cnt_pn4_s = row4["_s_cnt"]
        post_s_pn3_s = row3["_post_s_cnt"]
