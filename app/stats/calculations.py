...
def get_csv_fingerprint(data_dir: str) -> str:
    """CSV ファイル群（data_dir 直下の *.csv）の現在のフィンガープリント。
    全 CSV の (mtime, size) を集めて短いハッシュ文字列にする。
    分布データ等のダウンストリームキャッシュ命名に使う。CSV を更新すると
    フィンガープリントが変わるので、古いキャッシュファイルは自然に無視される。"""
    import os, hashlib
    ...
    for f in sorted(os.listdir(data_dir)):
        ...
        try:
            st = os.stat(p)
            sig.append(f"{f}:{int(st.st_mtime)}:{st.st_size}")
        except Exception:
            sig.append(f"{f}:0:0")
    if not sig:
        return "empty"
    h = hashlib.sha1("|".join(sig).encode("utf-8")).hexdigest()[:16]
    return h
...
def calculate_pitcher_overall_pitch_metrics(df: pd.DataFrame) -> dict:
    ...
    # 平均球速（★ ストレートのみの平均球速。指標順位検索の順位付けに使う数値）
    avg_velo = float("nan")
    if "球速" in df.columns and "球種" in df.columns:
        ff_v = pd.to_numeric(df.loc[df["球種"].astype(str) == "ストレート", "球速"],
                             errors="coerce")
        ff_pos = ff_v[ff_v > 0]
        if not ff_pos.empty:
            avg_velo = round(float(ff_pos.mean()), 1)
...
def calculate_first_strike_swing_rate(df: pd.DataFrame) -> dict:
    ...
    ★ 高速化版：astype(str) を1回だけにし、ブール演算で集計"""
    default = {"全打席": 0.0, "ストレート": 0.0, "変化球": 0.0, "カーブ": 0.0}
    if df.empty:
        return default
    if "ストライクカウント" not in df.columns or "投球結果" not in df.columns:
        return default

    sc = pd.to_numeric(df["ストライクカウント"], errors="coerce")
    is_pitch = (df["球種"].notna() if "球種" in df.columns
    ...
    pr = sub["投球結果"].astype(str)
    bq = (sub["打球性質"].astype(str)
          if "打球性質" in sub.columns
          else pd.Series("", index=sub.index))
    swing = (bq.isin(["ゴロ", "ライナー", "フライ"])
             | pr.eq("空振り") | pr.eq("ファウル"))

    pt = (sub["球種"].astype(str)
    ...
    def _rate(mask_pt):
        ...
        if n == 0:
            return 0.0
        return round(float(swing[mask_pt].sum() / n * 100), 1)

    return {
        "全打席":    _rate(pd.Series(True, index=sub.index)),
        "ストレート": _rate(pt == "ストレート"),
        "変化球":    _rate(pt.isin(breaking)),
        "カーブ":    _rate(pt == "カーブ"),
    }
...
def calculate_two_strike_metrics(df: pd.DataFrame) -> dict:
    ...
    ★ 高速化版：打者単位で呼ばれるため、ローカルな打席ID（_extract_final_pitches_per_pa と
       同じ方式）で十分。game_id ベースの groupby を完全に排除する。
    """
    ...
    if "ストライクカウント" not in df.columns or "球数" not in df.columns \
       or "投球結果" not in df.columns or "打席結果" not in df.columns:
        return default

    # 各種列を一度に取得（数値化はメモリ削減のため float32）
    p_balls = pd.to_numeric(df["球数"], errors="coerce")
    p_S     = pd.to_numeric(df["ストライクカウント"], errors="coerce")
    pa_res  = df["打席結果"]
    is_end  = pa_res.notna()

    # ローカル打席 ID（打席結果が立った行で打席終了 → shift して cumsum）
    # ※ _extract_final_pitches_per_pa と同じ思想
    is_final = pa_res.notna() & (~pa_res.astype(str).isin(["投球", "牽制"]))
    pa_id = is_final.shift(1, fill_value=True).cumsum()

    # 投球行
    is_pitch = (df["球種"].notna() if "球種" in df.columns
                else pd.Series(True, index=df.index))

    pr  = df["投球結果"].astype(str)
    bq  = (df["打球性質"].astype(str)
           if "打球性質" in df.columns
           else pd.Series("", index=df.index))
    bip   = bq.isin(["ゴロ", "ライナー", "フライ"])
    whiff = pr.eq("空振り")
    foul  = pr.isin(["ファウル", "ファール"])
    swing = bip | whiff | foul

    if "投球コース" in df.columns:
        course = pd.to_numeric(df["投球コース"], errors="coerce")
        o_zone = course.between(10, 25, inclusive="both")
    else:
        o_zone = pd.Series(False, index=df.index)

    # 投球行だけに絞る
    mask = is_pitch.values
    if not mask.any():
        return default

    df_p = pd.DataFrame({
        "pa":    pa_id.values[mask],
        "balls": p_balls.values[mask],
        "S":     p_S.values[mask],
        "swing": swing.values[mask],
        "foul":  foul.values[mask],
        "ozone": o_zone.values[mask],
        "end":   is_end.values[mask],
    })

    # 各打席ごとの「2S到達球数 (min)」「結果球の球数 (max)」を groupby agg で計算
    is_2s = df_p["S"] == 2
    if not is_2s.any():
        return default

    s_per_pa = df_p.loc[is_2s].groupby("pa", sort=False)["balls"].min()

    end_rows = df_p[df_p["end"]]
    if not end_rows.empty:
        e_per_pa = end_rows.groupby("pa", sort=False)["balls"].max()
    else:
        e_per_pa = df_p.groupby("pa", sort=False)["balls"].max()

    # 共通の打席だけ残す
    common = s_per_pa.index.intersection(e_per_pa.index)
    if len(common) == 0:
        return default
    s_per_pa = s_per_pa.loc[common]
    e_per_pa = e_per_pa.loc[common]

    valid = s_per_pa.notna() & e_per_pa.notna() & (s_per_pa <= e_per_pa)
    s_per_pa = s_per_pa[valid]
    e_per_pa = e_per_pa[valid]
    if s_per_pa.empty:
        return default

    avg_pitches = float((e_per_pa - s_per_pa + 1).astype(float).mean())

    # 各行に「自分の打席の 2S 到達球数 / 結果球数」を map で貼る
    df_p["s_at"] = df_p["pa"].map(s_per_pa)
    df_p["e_at"] = df_p["pa"].map(e_per_pa)

    after_mask = (df_p["s_at"].notna() & df_p["e_at"].notna()
                  & (df_p["balls"] >= df_p["s_at"])
                  & (df_p["balls"] <= df_p["e_at"]))
    after = df_p[after_mask]

    if after.empty:
        return {"2s後の平均投球数": round(avg_pitches, 2),
                "2s後の平均ファウル数": 0.0, "2s後のO-Swing%": 0.0}

    # ファウル数：2S 到達した状態でのファウル
    fouls = (after.loc[(after["S"] == 2) & after["foul"]]
             .groupby("pa", sort=False).size())
    avg_fouls = float(fouls.reindex(s_per_pa.index, fill_value=0).mean())

    # O-Swing% : 2S以降の boll-zone での swing
    o_pitches = int(after["ozone"].sum())
    o_swings  = int((after["ozone"] & after["swing"]).sum())
