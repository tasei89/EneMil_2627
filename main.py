...
with st.sidebar:
    ...
    # ── 対象年セレクタ（全ページ共通） ──
    # CSV から年候補を抽出
    from app.stats.calculations import load_csvs as _load_csvs, _normalize_date_series
    import os as _os
    DATA_DIR_FOR_YEARS = _os.environ.get("DATA_DIR", "data")
    _df_for_years = _load_csvs(DATA_DIR_FOR_YEARS)
    year_opts = ["すべて"]
    # ★ load_csvs が事前計算した _year 列を優先使用（pd.to_datetime をskipして高速化）
    if not _df_for_years.empty and "_year" in _df_for_years.columns:
        ...
        # フォールバック：_year 列がなければ旧来通り pd.to_datetime
    ...
    # ── データ再読込ボタン ──
    # CSVを更新したのに反映されない場合、全キャッシュをクリアして強制再読込する。
    # data/.cache/* のディスクキャッシュも、@st.cache_data のキャッシュも全部削除。
    if st.button("🔄 データを再読込", use_container_width=True,
                 help="CSVを更新したのに反映されない場合に押す"):
        # Streamlit のキャッシュをクリア
        try:
            st.cache_data.clear()
        except Exception:
            pass
        # ディスクキャッシュも削除（強制的に CSV から再構築させる）
        import shutil as _shutil
        _cache_dir = _os.path.join(DATA_DIR_FOR_YEARS, ".cache")
        ...
        # プロセス内キャッシュも明示的にクリア
        try:
            from app.stats import calculations as _calc
            _calc._CSV_CACHE["key"] = None
            _calc._CSV_CACHE["df"] = None
        except Exception:
            pass
        st.success("キャッシュをクリアしました。ページが再読込されます。")
        st.rerun()
