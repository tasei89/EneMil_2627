...
with st.sidebar:
    ...
    # get_csv_fingerprint は新しい calculations.py にのみ存在するため防御的に import
    try:
        from app.stats.calculations import get_csv_fingerprint as _gfp
    except ImportError:
        _gfp = None
    import os as _os
    ...
    _fp_now = _gfp(DATA_DIR_FOR_YEARS) if _gfp else None
    if (_fp_now is not None and _YEAR_OPTS_CACHE["fp"] == _fp_now
            and _YEAR_OPTS_CACHE["opts"] is not None):
